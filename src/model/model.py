...
class VanillaGAN(nn.Module):
    ...
    def visualize_distribution(self, epoch: int, batch_size: int, dataloader: object, writer: object) -> None:
        ...
        # Forward pass to get fake sample
        fake_sample = self.generator(z)
        writer.add_histogram('Inferred Distribution', values=config.normalize_tensor(fake_sample), global_step=epoch, bins=256)
        ...
        writer.add_histogram('Actual Distribution', values=config.normalize_tensor(real_sample), global_step=epoch, bins=256)
